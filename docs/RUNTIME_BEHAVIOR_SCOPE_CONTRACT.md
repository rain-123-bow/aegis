...
Before issuing or accepting a Seal, the definition and the four fixed approval artifacts (review report, review result, user confirmation, decision) must exist in that governed commit and match its exact blob bytes. Ignored, untracked, staged-only, or locally replaced approval evidence cannot satisfy commit membership. These files are approval-control inputs, not a selection of `.aegis` as runtime code; their current bytes remain share-locked throughout the check. The approval chain and runtime file set are re-resolved after lock acquisition, and their policy digest, decision digest, and entries must match the initial resolution. An internally inconsistent committed chain cannot pass by presenting different local review bytes before the locks are acquired.
