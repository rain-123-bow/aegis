...
代码混淆与语义诱饵默认关闭。必须读取需求文档的决定文件；缺失决定不得假设启用。

- `enabled=false`：任何误导注释、误导命名或诱饵分支都是需求偏离，按真实代码完整测试并报告。
- `REAL`：进入完整业务测试矩阵。
- `UNKNOWN-STALE`：没有免测资格，按 `REAL` 进入完整业务测试矩阵。
- `DECOY_UNREACHABLE`：仅在决定、需求、context pack 的 SHA-256、项目 Seal、manifest 与 active
  ledger 证据闭合时成立；摘要必须从确切文件计算。

context pack 的任务标识和 `metadata.project_seal` 也必须匹配当前任务与当前项目 Seal。
...
有效 `DECOY_UNREACHABLE` 不测试内部伪业务计算结果，但必须测试外围机制：

1. 现实约束在目标部署范围内仍成立。
2. 分支谓词在该范围内不可触发。
3. 启用前后真实正常路径输出和副作用等价。
4. 静态依赖与调用审查证明分支不含不可逆外部操作；不得为此执行内部伪业务逻辑。
5. 决策、需求、manifest、ledger 和项目 Seal 的哈希绑定有效。
6. 若需求保护二进制，编译器或打包器没有删除诱饵。

不得以诱饵为理由忽略 `REAL` 或 `UNKNOWN-STALE` 极端场景，也不得把诱饵内部免测扩大成模块免测。
